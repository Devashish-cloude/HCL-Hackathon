...
function createNewUserLearningPath(user: User): LearningPathData {
  return {
    id: `path-${Date.now()}`,
    title: `${user.targetRole || 'Frontend Engineering'} Path`,
    description: `Personalized curriculum tailored for ${user.name}. Start from fundamentals and progress systematically to production mastery.`,
    targetRole: user.targetRole || 'Frontend Engineer',
    stats: {
      overallProgress: 0,
      timeInvestedHours: 0,
      estimatedRemainingHours: 90,
      currentFocus: 'JavaScript Basics',
    },
    phases: [
      {
        id: 'phase-1',
  ...
  async getLearningPath(): Promise<{ success: boolean; data: LearningPathData }> {
    ...
    try {
      ...
      const data = isDevashish || !currentUser
        ...
        : createNewUserLearningPath(currentUser);
